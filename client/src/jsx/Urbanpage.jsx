...
function UrbanWrap() {
  ...
  const categoryOrder = ["VEG WRAPS", "NON-VEG WRAPS", "VEG PASTA", "NON-VEG PASTA", "FRIES", "GARLIC BREAD", "MILKSHAKES", "MOJITOS", "DESSERTS"];
  ...
  const customizationGroups = [
    {
      id: "base", title: "Choose Your Base", subtitle: "Select upto 1", maxSelection: 1, required: false,
      options: [
    ...
    {
      id: "extras", title: "Extra Toppings", subtitle: "Select upto 3", maxSelection: 3, required: false,
      options: [
    ...
    {
      id: "sides", title: "Add Sides", subtitle: "Select upto 2", maxSelection: 2, required: false,
      options: [
      ...
      try {
        ...
        sortedCategories.forEach(category => {
          category.items.forEach(item => {
            allItems.push({ ...item, category: category.category, itemId: item.id || `${category.category}-${item.name}`.replace(/\s+/g, '_').toLowerCase() });
          });
        });
  ...
  useEffect(() => {
    ...
    if (sortType === "rating") {
      filtered = filtered.map(category => ({ ...category, items: category.items.filter(item => parseFloat(item.rating) >= 4.0) })).filter(category => category.items.length > 0);
      ...
      const priority = bestsellerItems.filter(i => i.categoryName.includes('WRAP') || i.categoryName.includes('PASTA'));
      const others = bestsellerItems.filter(i => !i.categoryName.includes('WRAP') && !i.categoryName.includes('PASTA'));
  ...
  const handleAddToCart = () => {
    ...
    addToCart({ id: selectedItem.itemId, name: selectedItem.name, price: selectedItem.price + addonPrice, img: selectedItem.img, qty: itemQty, addons: addonsList, basePrice: selectedItem.price });
  ...
  const generatePairingSuggestions = (addedItem) => {
    ...
    let pairings = [];
    if (addedItem.category.includes('WRAP')) pairings = allItems.filter(item => item.isAvailable !== false && (item.category.includes('FRIES') || item.category.includes('SHAKE') || item.category.includes('MOJITO'))).slice(0, 3);
    else if (addedItem.category.includes('FRIES')) pairings = allItems.filter(item => item.isAvailable !== false && (item.category.includes('WRAP') || item.name.toLowerCase().includes('dip'))).slice(0, 3);
  ...
  // ✅ KITCHEN CLOSED GATE — checks global kitchen AND urbanwrap brand toggle
  if (statusLoading) return null;
  if (!isBrandOpen("urbanwrap")) return <KitchenClosed />;
        ...
        <div className="top-picks-section">
          ...
          <div className="top-picks-grid">
            {topPicks.map((item) => {
              ...
              const isAvailable = item.isAvailable !== false;
              return (
                <div key={item.itemId} className="top-pick-card" style={{ opacity: isAvailable ? 1 : 0.6 }}>
                  ...
                  <div className="top-pick-content">
                    <div className="top-pick-header">{renderFoodTypeIndicator(item)}<h4 className="top-pick-name">{item.name}</h4></div>
                    ...
                    {!isAvailable ? (
                      ...
                      <button className="top-pick-add-btn" onClick={() => handleItemClick(item, item.category, item.itemId)}>ADD</button>
                    ) : (
                ...
                return (
                  <div key={item.itemId} className="pairing-card">
                    ...
                    <div className="pairing-details">
                      ...
                      {qty === 0 ? (
                        <button className="pairing-add-btn" onClick={() => addToCart({ id: item.itemId, name: item.name, price: item.price, img: item.img, qty: 1, basePrice: item.price })}>ADD +</button>
                      ) : (
