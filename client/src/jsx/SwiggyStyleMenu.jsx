// src/jsx/SwiggyStyleMenu.jsx
// ✅ STEP 2 + STEP 3: Kitchen closed gate + per-item out-of-stock support
...
import { useRestaurantStatus } from "../context/RestaurantStatusContext"; // ✅ STEP 2
import KitchenClosed from "./KitchenClosed";                              // ✅ STEP 2
import "./SwiggyStyleMenu.css";

function SwiggyStyleMenu() {
  const { isBrandOpen, loading: statusLoading } = useRestaurantStatus(); // ✅ STEP 2
  ...
  const categoryOrder = ["VEG BURGERS", "NON-VEG BURGERS", "SANDWICH", "WRAPS", "FRIES", "SHAKES", "MOJITOS"];
  ...
  const customizationGroups = [
    {
      id: "meal", title: "Make It A Meal", subtitle: "Select upto 1", maxSelection: 1, required: false,
      options: [{ id: "meal-1", name: "Meal (fries + Lime N Mint)", price: 129, type: "veg" }]
    },
    {
      id: "patty", title: "Add Fried Patty To Your Burger", subtitle: "Select upto 2", maxSelection: 2, required: false,
      options: [
    ...
    {
      id: "cheese", title: "Cheese Please!", subtitle: "Select upto 1", maxSelection: 1, required: false,
      options: [
    ...
    {
      id: "sides", title: "Sides & Dips", subtitle: "Select upto 3", maxSelection: 3, required: false,
      options: [
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        ...
        setTopPicks(allItems.filter(i => parseFloat(i.rating) >= 4.5).sort((a, b) => parseFloat(b.rating) - parseFloat(a.rating)).slice(0, 6));
        setDiscountedItems(allItems.filter(i => i.discount && i.discount > 0).sort((a, b) => b.discount - a.discount).slice(0, 6));
        setRecommendedItems(allItems.filter(i => parseFloat(i.rating) >= 4.3 && i.isAvailable !== false).sort((a, b) => parseFloat(b.rating) - parseFloat(a.rating)).slice(0, 6));
        setCategoryBanners(sortedCategories.map(category => ({
          category: category.category,
          startingPrice: Math.min(...category.items.map(i => i.price)),
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setActiveDealIndex((prev) => prev === offers.length - 1 ? 0 : prev + 1), 1500);
  ...
  useEffect(() => {
    ...
    if (sortType === "rating") {
      filtered = filtered.map(category => ({
        ...category, items: category.items.filter(item => parseFloat(item.rating) >= 4.0)
      })).filter(category => category.items.length > 0);
    } else if (sortType === "bestseller") {
      const allItems = [];
      filtered.forEach(category => { category.items.forEach(item => allItems.push({ ...item, categoryName: category.category })); });
      let bestsellerItems = allItems.filter(item => parseFloat(item.rating) >= 4.5);
      const priority = bestsellerItems.filter(i => i.categoryName.includes('BURGER') || i.categoryName.includes('SANDWICH'));
      const others = bestsellerItems.filter(i => !i.categoryName.includes('BURGER') && !i.categoryName.includes('SANDWICH'));
      bestsellerItems = [...priority, ...others].slice(0, 11);
      const categoryMap = new Map();
      bestsellerItems.forEach(item => {
        if (!categoryMap.has(item.categoryName)) categoryMap.set(item.categoryName, { category: item.categoryName, items: [] });
  ...
  const handleItemClick = (item, category, itemId) => {
    // ✅ STEP 3: Don't open modal for out-of-stock items
  ...
  const handleAddToCart = () => {
    ...
    addToCart({ id: selectedItem.itemId, name: selectedItem.name, price: selectedItem.price + addonPrice, img: selectedItem.img, qty: itemQty, addons: addonsList, basePrice: selectedItem.price, brand: "Shrimmers" });
  ...
  const generatePairingSuggestions = (addedItem) => {
    ...
    menuData.forEach(category => {
      category.items.forEach(item => allItems.push({ ...item, category: category.category, itemId: item.id || `${category.category}-${item.name}`.replace(/\s+/g, '_').toLowerCase() }));
    });
    let pairings = [];
    if (addedItem.category.includes('BURGER')) pairings = allItems.filter(item => item.isAvailable !== false && (item.category.includes('FRIES') || item.category.includes('SHAKE') || item.category.includes('MOJITO'))).slice(0, 3);
    else if (addedItem.category.includes('FRIES')) pairings = allItems.filter(item => item.isAvailable !== false && (item.category.includes('BURGER') || item.name.toLowerCase().includes('dip'))).slice(0, 3);
    else pairings = allItems.filter(item => item.isAvailable !== false && item.category !== addedItem.category && parseFloat(item.rating) >= 4.3).sort((a, b) => parseFloat(b.rating) - parseFloat(a.rating)).slice(0, 3);
  ...
  const handleFilterClick = (type) => {
    if (filterType === type) setFilterType("all");
      ...
      setFilterType(type); setSortType("");
      setTimeout(() => document.querySelector(".menu-title-divider")?.scrollIntoView({ behavior: "smooth", block: "start" }), 300);
    }
  };
  ...
  // ✅ STEP 2: Kitchen closed gate — checks global kitchen AND shrimmers brand toggle
  if (statusLoading) return null;
  if (!isBrandOpen("shrimmers")) return <KitchenClosed />;
    ...
    <div className="menu-wrapper">
      <div className="top-search-bar" ref={searchBarRef}>
        <div className="top-search-wrapper">
          ...
          <button className={`veg-only-toggle ${filterType === "veg" ? "active" : ""}`} onClick={() => handleFilterClick("veg")}>
            ...
            <div key={offers[activeDealIndex].id} className="deal-strip-content">
              <div className="deal-left">
                ...
                <div className="deal-text-wrap">
                  <p className="deal-main-text">{offers[activeDealIndex].title}</p>
                  <p className="deal-sub-text">USE <strong>{offers[activeDealIndex].code}</strong></p>
                </div>
              </div>
      ...
      <div className="filter-section">
        <div className="filter-buttons">
          <button className={`filter-btn ${filterType === "all" && sortType === "" ? "active" : ""}`} onClick={() => { setFilterType("all"); setSortType(""); }}>
            <span className="filter-text">All</span><span className="filter-count">{totalItems}</span>
          </button>
          <button className={`filter-btn veg-filter ${filterType === "veg" ? "active" : ""}`} onClick={() => handleFilterClick("veg")}>
            <span className="veg-indicator"><span className="veg-dot"></span></span>
            <span className="filter-text">Veg</span><span className="filter-count">{vegItems}</span>
          </button>
          <button className={`filter-btn nonveg-filter ${filterType === "nonveg" ? "active" : ""}`} onClick={() => handleFilterClick("nonveg")}>
          ...
          <button className={`filter-btn sort-btn ${sortType === "rating" ? "active" : ""}`} onClick={() => setSortType(sortType === "rating" ? "" : "rating")}>
            <span className="filter-text">⭐ 4.0+</span>
          </button>
          <button className={`filter-btn sort-btn ${sortType === "bestseller" ? "active" : ""}`} onClick={() => setSortType(sortType === "bestseller" ? "" : "bestseller")}>
              ...
              return (
                <div key={item.id} className="order-again-card" style={{ opacity: isAvailable ? 1 : 0.5 }}>
                  ...
                  {!isAvailable ? (
                    ...
                    <button className="discount-add-btn" onClick={() => handleItemClick(item, item.category, item.itemId)}>ADD +</button>
                  ) : (
          ...
          {categoryBanners.map((banner, idx) => {
            ...
            return (
              <div key={idx} className="category-banner-wrapper">
                <div className="category-banner-card" onClick={() => setExpandedBanner(isExpanded ? null : idx)}>
                ...
                {isExpanded && category && (
                  <div className="banner-items-dropdown">
                      ...
                      return (
                        <div key={itemIdx} className="banner-dropdown-item">
                          ...
                          {!isAvailable ? (
                            ...
                            <button className="banner-item-add" onClick={() => handleItemClick(item, category.category, itemId)}>ADD</button>
                          ) : (
                    ...
                    {category.items.length > 4 && <p className="view-all-text">+ {category.items.length - 4} more items</p>}
                  </div>
                )}
              </div>
            );
              ...
              return (
                <div key={item.itemId} className="recommended-card">
                  ...
                  <div className="recommended-content">
                    <div className="recommended-header">{renderFoodTypeIndicator(item)}<h4 className="recommended-name">{item.name}</h4></div>
                    <div className="recommended-info">
                      <span className="recommended-rating">⭐ {item.rating}</span>
                      <span className="recommended-price">₹{item.price}</span>
                    </div>
                    {qty === 0 ? (
                      <button className="recommended-add-btn" onClick={() => handleItemClick(item, item.category, item.itemId)}>ADD</button>
                    ) : (
      ...
      {loading ? (
        ...
        <div className="empty-filter">
          ...
          <button className="reset-filter-btn" onClick={() => { setFilterType("all"); setSearchQuery(""); }}>Show All Items</button>
        </div>
      ) : (
              ...
              {isOpen && (
                <div className="item-list">
                  {section.items.map((item, index) => {
                    ...
                    const qty = getItemQty(itemId);
                    // ✅ STEP 3: Check isAvailable (default true if not set)
                    const isAvailable = item.isAvailable !== false;
                      ...
                      <div key={index} className="menu-item-card" style={{ opacity: isAvailable ? 1 : 0.6 }}>
                        <div className="item-info">
                          <div style={{ display: "flex", alignItems: "center", marginBottom: "8px" }}>
                            ...
                            {/* ✅ STEP 3: Out of stock badge */}
                            {!isAvailable && (
                              <span style={{
                                marginLeft: "8px", fontSize: "10px", fontWeight: "800",
                                color: "#f44336", background: "rgba(244,67,54,0.12)",
                                padding: "2px 8px", borderRadius: "4px", border: "1px solid rgba(244,67,54,0.3)"
                              }}>OUT OF STOCK</span>
                            )}
                          </div>
                        ...
                        <div className="item-img-wrapper">
                          <img src={item.img} className="item-img" alt={item.name} style={{ filter: isAvailable ? "none" : "grayscale(60%)" }} />
                          {/* ✅ STEP 3: Show disabled state for out-of-stock */}
                          {!isAvailable ? (
                            <button className="add-btn" disabled style={{ background: "#444", cursor: "not-allowed", color: "#888" }}>
                              N/A
                            </button>
                          ) : qty === 0 ? (
      ...
      {showPairingModal && pairingSuggestions.length > 0 && (
        <div className="pairing-modal-overlay" onClick={() => setShowPairingModal(false)}>
          <div className="pairing-modal" onClick={(e) => e.stopPropagation()}>
                ...
                return (
                  <div key={item.itemId} className="pairing-card">
                    ...
                    <div className="pairing-details">
                      ...
                      {qty === 0 ? (
                        <button className="pairing-add-btn" onClick={() => { addToCart({ id: item.itemId, name: item.name, price: item.price, img: item.img, qty: 1, basePrice: item.price, brand: "Shrimmers" }); }}>ADD +</button>
                      ) : (
            ...
            <button className="continue-btn" onClick={() => setShowPairingModal(false)}>Continue →</button>
          </div>
        </div>
      )}
    </div>
